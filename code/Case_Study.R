...
steph_curry_3PA <- as.vector(steph_curry_stats$`3PA`)
...
# 3-Pt Percentage
steph_curry_3PP <- as.vector(steph_curry_stats$`3P%`)
...
steph_curry_ratio <- as.vector(steph_curry_stats$`3PA`)
...
# 3-Pt Attempts
draymond_green_3PA <- as.vector(draymond_green_stats$`3PtA`)
draymond_green_3PA <- as.numeric(draymond_green_3PA)
# 3-Pt Percentage
draymond_green_3PP <- as.vector(draymond_green_stats$`3P%`)
...
# 3-Pt Attempts
tim_duncan_3PA <- as.vector(tim_duncan_stats$`3PtA`)
tim_duncan_3PA <- as.numeric(tim_duncan_3PA)
# 3-Pt Percentage
tim_duncan_3PP <- as.vector(tim_duncan_stats$`3P%`)
...
barplot(case_study_df$heights, names.arg = cs_players, ylim = c(0, 85), las = 2,
  ylab = 'Height (inches)', main = 'Players Heights')
barplot(case_study_df$weights, names.arg= cs_players, las = 2, ylab = 'Weight (lbs)',
  ylim = c(0, 250), main = 'Players Weights')
barplot(case_study_df$X3PA, names.arg = case_study_df$cs_players, las = 2, 
  ylim = c(0, 10), ylab = '3-Pt Attempts', main = '3-Pt Attempts per Game')
barplot(case_study_df$X3PP, names.arg = case_study_df$cs_players, las = 2, 
